...
Verify_Expval_1 "$AP_IP" "tmp" "ls" "$APLOGINCHECK" "Verify Logo"

sleep 5

if [[ "$CMResult" == *"$APLOGINCHECK"* ]]; then
...
if [[ "$CMResult" == *"$SUB"* || "$CMResult" != *"$APLOGINCHECK"*  ]]; then
